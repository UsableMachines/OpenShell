...
`state:validated` means the factual assessment is complete and awaits human disposition. A human declines by closing the issue as not planned with a rationale, or accepts by replacing `state:validated` with `state:accepted` and placing the issue on the roadmap as documented in `CONTRIBUTING.md`. Accepted work may remain human-owned. A maintainer can request deeper agent investigation or planning with `agent:plan-requested`.

The optional agent workflow remains human-gated: `agent:plan-requested` authorizes planning, and `agent:implementation-requested` authorizes implementation after plan review.
...
> To delegate investigation or planning to an agent, also apply
> `agent:plan-requested` and route it to `create-spike` or
> `build-from-issue`. If no, close it as not planned and record the rationale.
...
Community issue filed
  ...
  create-spike          (if deeper investigation is approved)
        |
  human applies agent:plan-requested
        |
  build-from-issue      (creates implementation plan)
        |
  human applies agent:implementation-requested
        |
  implementation
```
...
- **build-from-issue** creates a plan after a human applies `agent:plan-requested`, then implements only after a human reviews the plan and applies `agent:implementation-requested`.
