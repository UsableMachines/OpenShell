...
> Review the issue and decide whether OpenShell should pursue it. If yes, replace `state:validated` with `state:accepted` and separately associate it with a roadmap item. The work may remain human-owned; to delegate planning to an agent, also apply `agent:plan-requested` and use `build-from-issue`. If no, close it as not planned and record the rationale.
