...
async fn install_generic_provider() -> Result<(), String> {
    ensure_provider_resources_absent().await?;
    let credential = format!("{TOKEN_ENV}={TEST_SECRET}");
    let (created, output) = run_cli(&[
        "provider",
    ...
    .await;
    if !created {
        return Err(format!("generic provider create failed:\n{output}"));
    }
    Ok(())
}
...
with socket.create_connection((proxy.hostname, proxy.port or 80), timeout=10) as sock:
    ...
    try:
        ...
        data = b""
    print("BINARY_DENIED" if not data else "BINARY_FORWARDED")
"#
    )
}
...
async fn run_body_sandbox(port: u16, rewrite: bool) -> Result<String, String> {
    let policy = write_policy(port, EndpointMode::RestBody { rewrite })?;
...
async fn assert_rest_body_backstop(server: &HttpProbeServer) -> Result<(), String> {
    let denied = run_body_sandbox(server.port, false).await?;
    assert!(denied.contains("BODY_DENIED"));

    let rewritten = run_body_sandbox(server.port, true).await?;
...
async fn credentialed_endpoint_gates_work_end_to_end() {
    ...
    install_generic_provider()
        .await
        .expect("install generic provider");
    ...
    .await;
    cleanup_provider_resources().await;
    generic_result.expect("generic provider REST backstop E2E");
}
