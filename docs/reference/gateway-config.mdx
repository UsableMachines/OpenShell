...
# acknowledgement because no credential is sent — setting one alongside it is
# rejected. Fail-closed: a sandbox whose identity is unresolved refuses to
# start rather than egressing unattributed.
# proxy_auth_sandbox_identity  = true
# Client certificate presented on the TLS hop to an https:// proxy that
# authenticates its callers with mutual TLS. Names an existing
# kubernetes.io/tls Secret in the sandbox namespace; the supervisor reads
# `tls.crt` and `tls.key` from a mount only the network supervisor container
# sees. Requires an https:// proxy URL — there is no TLS hop to present it on
# otherwise — and `topology = "sidecar"`, for the same fsGroup reason as the
# credential Secret.
# proxy_client_cert_secret_name = "corporate-proxy-client"
...
# because no credential is sent — setting one alongside it is rejected.
# Fail-closed: a sandbox whose identity is unresolved refuses to start rather
# than egressing unattributed.
# proxy_auth_sandbox_identity = true
```
