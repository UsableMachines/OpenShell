...
# sandbox_uid = 1500
# sandbox_gid = 1500
# Corporate forward proxy for sandbox egress. The network supervisor chains
# policy-approved TLS tunnels through this proxy with HTTP CONNECT instead of
# dialing destinations directly. Semantics, NO_PROXY matching, the CONNECT
# target warning, and the fail-closed behaviour are identical to the Podman
# driver's settings documented under "Podman" below; only credential delivery
# differs.
#
# In combined topology the flags land on the agent container's supervisor; in
# sidecar topology they land on the network sidecar. The process-only
# supervisor never dials upstream destinations and is deliberately left
# without them.
#
# Credentials are supplied as an existing Secret in the sandbox namespace
# rather than a gateway-host file path: the credential must be stored under
# the "upstream-proxy" key in the user:pass form, and kubelet projects it
# into the pod at /etc/openshell/auth/upstream-proxy. The gateway therefore
# never reads or re-delivers the credential bytes. Create it with:
#   kubectl -n <sandbox-namespace> create secret generic \
#     openshell-upstream-proxy-auth --from-literal=upstream-proxy='user:pass'
#
# WARNING: the supervisor sends the credential as a Proxy-Authorization:
# Basic header over the plain-TCP connection to the http:// proxy. Basic auth
# is base64, not encryption: anyone on the network path between the sandbox
# pod and the proxy can recover the credential. Because of that,
# proxy_auth_secret_name requires the explicit acknowledgement
# proxy_auth_allow_insecure = true; without it the configuration is rejected
# at gateway startup. Only opt in when the path to the proxy is a trusted
# network segment.
...
# Identify each sandbox to the proxy by sending its sandbox id as the
# Proxy-Authorization Basic username with an empty password, so the proxy can
# attribute and ACL egress per sandbox. The username is an identifier, not a
# secret: the supervisor builds the header on the upstream hop, so a workload
# cannot forge or alter it, but anything with direct network access to the
# proxy can — the proxy must not treat the value as authorization on its own.
# Mutually exclusive with proxy_auth_secret_name, and it needs no cleartext
# acknowledgement because no credential is sent. Fail-closed: a sandbox whose
# identity is unresolved refuses to start rather than egressing unattributed.
...
# Corporate forward proxy for sandbox egress. When set, the sandbox's network
...
# cannot forge or alter it, but anything with direct network access to the
# proxy can — the proxy must not treat the value as authorization on its own.
# Mutually exclusive with proxy_auth_file (a request carries one
# Proxy-Authorization header), and it needs no cleartext acknowledgement
# because no credential is sent. Fail-closed: a sandbox whose identity is
# unresolved refuses to start rather than egressing unattributed.
# proxy_auth_sandbox_identity = true
```
