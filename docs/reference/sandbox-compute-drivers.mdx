...
For proxy-required networks, the Podman driver also accepts the corporate egress proxy keys `https_proxy`, `no_proxy`, `proxy_auth_file`, `proxy_auth_allow_insecure`, and `proxy_connect_by_hostname`. It also accepts `proxy_auth_sandbox_identity`, which sends the sandbox id as the Basic-auth username so the proxy can attribute egress per sandbox instead of authenticating with a shared credential. The supervisor chains policy-approved TLS tunnels through the proxy with HTTP CONNECT instead of dialing destinations directly. See the [Gateway Configuration File](./gateway-config) reference for the full contract, including the cleartext-credential acknowledgement and the validated-IP CONNECT behavior.
...
| `proxy_auth_sandbox_identity` | `upstreamProxy.authSandboxIdentity` | Send the sandbox id as the `Proxy-Authorization: Basic` username with an empty password, so the proxy can attribute and ACL egress per sandbox. The username is an identifier, not a secret: the supervisor builds the header on the upstream hop, so a workload cannot forge or alter it, but anything with direct network access to the proxy can — the proxy must not treat the value as authorization on its own. Mutually exclusive with `proxy_auth_secret_name`, and it rejects `proxy_auth_allow_insecure` because no credential is sent. Fail-closed: a sandbox whose identity is unresolved refuses to start rather than egressing unattributed. |
| `proxy_client_cert_secret_name` | `upstreamProxy.clientCertSecret.name` | Existing `kubernetes.io/tls` Secret in the sandbox namespace whose `tls.crt` and `tls.key` the supervisor presents on the TLS hop to a proxy that authenticates its callers with mutual TLS. Requires an `https://` proxy URL and `sidecar` topology. |
