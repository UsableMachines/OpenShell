...
For proxy-required networks, the Podman driver also accepts the corporate egress proxy keys `https_proxy`, `no_proxy`, `proxy_auth_file`, `proxy_auth_allow_insecure`, and `proxy_connect_by_hostname`. The Kubernetes driver accepts the same keys, except that credentials come from a namespace `Secret` through `proxy_auth_secret_name` instead of a gateway-host file path. Both drivers also accept `proxy_auth_sandbox_identity`, which sends the sandbox id as the Basic-auth username so the proxy can attribute egress per sandbox instead of authenticating with a shared credential. The supervisor chains policy-approved TLS tunnels through the proxy with HTTP CONNECT instead of dialing destinations directly. See the [Gateway Configuration File](./gateway-config) reference for the full contract, including the cleartext-credential acknowledgement and the validated-IP CONNECT behavior.
...
| `https_proxy` | `server.upstreamProxy.url` | Chain policy-approved TLS egress through a corporate forward proxy with HTTP CONNECT instead of dialing destinations directly. Only `http://host:port` form is accepted; the scheme and port are both required. Empty disables upstream proxying. |
| `no_proxy` | `server.upstreamProxy.noProxy` | Comma-separated list of destinations dialed directly (hostnames, domain suffixes, IPs, CIDRs, each with an optional `:port` qualifier). Rejected without a proxy URL. |
| `proxy_auth_secret_name` | `server.upstreamProxy.authSecretName` | Name of an existing Secret in the sandbox namespace holding the proxy credential as `user:pass` under the `upstream-proxy` key. Kubelet projects it into the pod at `/etc/openshell/auth/upstream-proxy`, so the gateway never reads the credential and it never appears on argv, in the environment, or in pod specs. This is the Kubernetes counterpart of the Podman driver's `proxy_auth_file`. |
| `proxy_auth_allow_insecure` | `server.upstreamProxy.authAllowInsecure` | Acknowledge that the credential is sent as cleartext Basic auth over the plain-TCP connection to the `http://` proxy. Required whenever `proxy_auth_secret_name` is set; the gateway refuses to start otherwise. |
| `proxy_connect_by_hostname` | `server.upstreamProxy.connectByHostname` | Send the destination hostname in CONNECT instead of a validated IP. Last resort for proxies whose ACLs filter on hostnames: the proxy resolves the name itself, so sandbox SSRF and `allowed_ips` validation no longer bind the connection and the proxy's ACLs become the effective egress control. |
| `proxy_auth_sandbox_identity` | `server.upstreamProxy.authSandboxIdentity` | Send the sandbox id as the `Proxy-Authorization: Basic` username with an empty password, so the proxy can attribute and ACL egress per sandbox. The username is an identifier, not a secret: the supervisor builds the header on the upstream hop, so a workload cannot forge or alter it, but anything with direct network access to the proxy can — the proxy must not treat the value as authorization on its own. Mutually exclusive with `proxy_auth_secret_name`. Fail-closed: a sandbox whose identity is unresolved refuses to start rather than egressing unattributed. |
...
When a corporate upstream proxy is configured, its flags are rendered onto
whichever container runs network supervision: the agent container in
`combined` topology, the network sidecar in `sidecar` topology. The
process-only supervisor never dials upstream destinations, so it receives
neither the flags nor the credential mount. Because the settings travel on the
supervisor's command line, a sandbox spec, template environment, or image
`ENV` cannot redirect or disable the boundary.
