...
fn gateway_listener_specs_with_default_route_ip(
    ...
    for requirement in requirements {
        ...
        if specs
            .iter()
            .any(|spec| listener_satisfies_loopback_interface(spec.address))
        {
            continue;
        }
...
mod tests {
    use super::{
        ConfiguredComputeDriver, ConnectionProtocol, GatewayListenerPurpose,
        GatewayListenerRequirement, GatewayListenerSpec, MultiplexService, ServerState,
        TlsAcceptor, allow_plaintext_service_http, bind_gateway_listeners, classify_initial_bytes,
        configured_compute_driver, gateway_listener_specs,
        gateway_listener_specs_with_default_route_ip, is_benign_tls_handshake_failure,
    ...
    #[test]
    fn gateway_listener_specs_skip_podman_exact_when_primary_covers_it() {
        let primary: SocketAddr = "0.0.0.0:8080".parse().unwrap();

        assert_eq!(
            gateway_listener_specs(
                primary,
                &[podman_listener_requirement(
                    "10.89.1.1:8080".parse().unwrap()
                )],
            )
            .unwrap(),
            vec![GatewayListenerSpec {
    ...
    fn gateway_listener_specs_resolve_podman_default_route_source() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                GatewayListenerSpec {
                    address: "192.168.20.20:8080".parse().unwrap(),
                    purpose: GatewayListenerPurpose::ComputeDriverCallback {
                        driver_name: "podman".to_string(),
    ...
    #[test]
    fn gateway_listener_specs_skip_default_route_when_primary_is_ipv4_wildcard() {
        let primary: SocketAddr = "0.0.0.0:8080".parse().unwrap();
    ...
    #[test]
    fn gateway_listener_specs_skip_podman_loopback_when_primary_covers_it() {
        for primary in ["127.0.0.1:8080", "0.0.0.0:8080"] {
