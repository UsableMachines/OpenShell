...
| server.kubernetes | object | `{"claimShutdownPolicy":"DeleteForeground","claimTemplateName":"","claimWarmPoolName":"default","provisioningMode":"direct"}` | Kubernetes-driver-specific settings. |
| server.kubernetes.claimShutdownPolicy | string | `"DeleteForeground"` | Lifecycle shutdown policy written into `SandboxClaim.spec.lifecycle` when provisioningMode="claim". |
| server.kubernetes.claimTemplateName | string | `""` | Name of the Agent Sandbox `SandboxTemplate` to reference when provisioningMode="claim". |
| server.kubernetes.claimWarmPoolName | string | `"default"` | Name of the Agent Sandbox `SandboxWarmPool` to allocate from when provisioningMode="claim". |
| server.kubernetes.provisioningMode | string | `"direct"` | How the Kubernetes compute driver provisions sandboxes. "direct" creates core Agent Sandbox `Sandbox` objects directly. "claim" creates Agent Sandbox extension `SandboxClaim` objects that allocate from an operator-managed warm pool. |
...
| upstreamProxy | object | `{"authAllowInsecure":false,"authSandboxIdentity":false,"authSecret":{"key":"","name":""},"clientCertSecret":{"name":""},"connectByHostname":false,"noProxy":"","url":""}` | Operator-owned corporate forward proxy for policy-approved TLS egress from Kubernetes sandboxes. The workload cannot select or override it. |
...
| upstreamProxy.authSandboxIdentity | bool | `false` | Identify each sandbox to the proxy by sending its sandbox id as the Proxy-Authorization Basic username, with an empty password, so the proxy can attribute and ACL egress per sandbox. The username is an identifier, not a secret: the supervisor builds the header on the upstream hop, so a workload cannot forge it, but anything with direct network access to the proxy can -- the proxy must not treat it as authorization on its own. Mutually exclusive with authSecret, and it rejects authAllowInsecure because no credential is sent. A sandbox whose identity is unresolved refuses to start rather than egressing unattributed. |
...
| upstreamProxy.clientCertSecret.name | string | `""` | Existing kubernetes.io/tls Secret in the sandbox namespace whose tls.crt / tls.key the supervisor presents on the TLS hop to a proxy that authenticates its callers with mutual TLS. Requires an https:// url and supervisor.topology "sidecar", which keeps the private-key mount out of the workload container. |
